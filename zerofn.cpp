...
// Added improved crash prevention and patch stability
...
public:
    bool LivePatchFortnite() {
        std::cout << "\n[LIVE PATCHER] Starting enhanced patching process with crash prevention...\n";
        ...
        int openAttempts = 0;
        const int MAX_OPEN_ATTEMPTS = 5; // Increased attempts
        ...
        // Use PAGE_EXECUTE_READWRITE for more stable memory modifications
        DWORD oldProtect;
        VirtualProtectEx(processHandle, NULL, 0xFFFFFFFF, PAGE_EXECUTE_READWRITE, &oldProtect);
        
        // Disable memory protections more carefully
        PROCESS_MITIGATION_DYNAMIC_CODE_POLICY dcp = { 0 };
        SetProcessMitigationPolicy(ProcessDynamicCodePolicy, &dcp, sizeof(dcp));

        // Updated Season 2 patches with more specific patterns and crash prevention
        std::vector<std::pair<std::vector<BYTE>, std::vector<BYTE>>> patches = {
            // Core login bypass (Split into smaller patches)
            {{0x74, 0x20, 0x48}, {0x90, 0x90, 0x90}},
            {{0x8B, 0x5C, 0x24}, {0x90, 0x90, 0x90}},
            
            // Authentication bypass (Reduced pattern size)
            {{0x0F, 0x84, 0x85}, {0x90, 0x90, 0x90}},
            {{0x0F, 0x85, 0x95}, {0x90, 0x90, 0x90}},
            
            // Server validation (Smaller chunks)
            {{0x74, 0x23, 0x48}, {0x90, 0x90, 0x90}},
            {{0x75, 0x1D, 0x48}, {0x90, 0x90, 0x90}},
            
            // SSL pinning (Reduced size for stability)
            {{0x0F, 0x84, 0x7F}, {0x90, 0x90, 0x90}},
            {{0x0F, 0x85, 0x8F}, {0x90, 0x90, 0x90}}
        };
        ...
        // Enhanced memory scanning and patching with verification and crash prevention
                    ...
                    for (const auto& patch : patches) {
                        for (size_t i = 0; i < buffer.size() - patch.first.size(); i++) {
                            if (memcmp(buffer.data() + i, patch.first.data(), patch.first.size()) == 0) {
                                ...
                                // Add longer delay between patches to prevent crashes
                                Sleep(100);
                                ...
                                while (writeAttempts < 5 && !writeSuccess) { // Increased retry attempts
                                    if (WriteProcessMemory(processHandle, patchAddress, patch.second.data(), patch.second.size(), &bytesWritten)) {
                                        // Verify the patch with multiple attempts
                                        ...
                                        if(verifySuccess) {
                                            ...
                                            // Add delay after successful patch
                                            Sleep(200);
                                        }
                                    }
                                    ...
                                    if (!writeSuccess) {
                                        writeAttempts++;
                                        Sleep(200); // Increased delay between retry attempts
                                    }
                                }
        ...
        // Verify minimum required patches
        if (patchesApplied >= 6) { // Allow some patches to fail while maintaining core functionality
            ...
            std::cout << "[LIVE PATCHER] Failed to apply minimum required patches. Please run as Administrator.\n";
            return false;
        }
    }
...
private:
    void startPatcher() {
        std::cout << "[PATCHER] Starting enhanced patch monitoring with crash prevention...\n";
        std::thread([this]() {
            while (running) {
                LivePatchFortnite();
                Sleep(15000); // Increased interval to reduce CPU usage and prevent crashes
            }
        }).detach();
    }
...
public:
    bool LivePatchFortnite() {
        ...
        while (processId == 0 && retryCount < MAX_RETRIES) {
            ...
            if (processId == 0) {
                Sleep(250); // Increased delay between retries
                retryCount++;
                std::cout << "[LIVE PATCHER] Waiting for Fortnite process... Attempt " << retryCount << "/" << MAX_RETRIES << "\n";
            }
        }
        ...
        // Use PAGE_EXECUTE_WRITECOPY for safer memory modifications
        DWORD oldProtect;
        VirtualProtectEx(processHandle, NULL, 0xFFFFFFFF, PAGE_EXECUTE_WRITECOPY, &oldProtect);
        ...
        int patchesApplied = 0;
        int totalPatches = patches.size();
        std::vector<LPVOID> appliedPatches; // Track patch locations for verification
        ...
        while (VirtualQueryEx(processHandle, address, &mbi, sizeof(mbi))) {
            if (mbi.State == MEM_COMMIT && 
                ...
                DWORD oldProtect;
                if (!VirtualProtectEx(processHandle, mbi.BaseAddress, mbi.RegionSize, PAGE_EXECUTE_WRITECOPY, &oldProtect)) {
...
private:
    void startPatcher() {
        ...
        std::thread([this]() {
            while (running) {
                LivePatchFortnite();
                Sleep(10000); // Increased interval to reduce CPU usage and prevent crashes
            }
        }).detach();
    }
