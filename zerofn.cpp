...
public:
    bool LivePatchFortnite() {
        ...
        std::vector<std::pair<std::vector<BYTE>, std::vector<BYTE>>> patches = {
            ...
            {{0x74, 0x23, 0x8B, 0x45, 0x08, 0x39}, {0x90, 0x90, 0x90, 0x90, 0x90, 0x90}},
            // Login check bypass
            {{0x84, 0xC0, 0x75, 0x1D, 0x8B, 0x45}, {0x90, 0x90, 0x90, 0x90, 0x90, 0x90}}
        };

        int patchesApplied = 0;
        DWORD oldProtect;

        // Scan and patch in chunks for speed
        const size_t CHUNK_SIZE = 0x1000;
        std::vector<BYTE> buffer(CHUNK_SIZE);

        for (size_t offset = 0; offset < moduleInfo.SizeOfImage; offset += CHUNK_SIZE) {
            SIZE_T bytesRead;
            if (ReadProcessMemory(processHandle, (LPCVOID)((DWORD_PTR)moduleInfo.lpBaseOfDll + offset),
                buffer.data(), CHUNK_SIZE, &bytesRead)) {
                            ...
                            if (VirtualProtectEx(processHandle, patchAddr, patch.second.size(), 
                                ...
                                if (WriteProcessMemory(processHandle, patchAddr, patch.second.data(),
                                    ...
                                    std::cout << "[LIVE PATCHER] Applied patch " << patchesApplied 
                                            << " at 0x" << std::hex << patchAddr << std::dec << "\n";
                                }
                            }
                        }
                    }
